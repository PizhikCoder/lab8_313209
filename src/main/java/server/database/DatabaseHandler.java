...
public class DatabaseHandler implements IDataLoader, IDataSaver, IDatabaseHandlerDAO {
    ...
    public int sendModel(MusicBand model, long modelId) {
        try {
            ...
            ResultSet result = dbConnection.sendQuery(RequestsEnum.SEND_MODEL_WITH_ID.createStatement(modelId, model.getOwnerId(), model.getName(), coordinates_id, Timestamp.valueOf(model.getCreationDate().toLocalDateTime()), model.getNumberOfParticipants(), genre_id, frontman_id));
            if (result.next()) {
                return result.getInt("id");
            }
            return -1;
