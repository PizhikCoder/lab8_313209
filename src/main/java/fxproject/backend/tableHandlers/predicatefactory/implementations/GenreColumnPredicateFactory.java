...
public class GenreColumnPredicateFactory extends PredicateFactory {
    ...
    private Predicate<MusicBand> createMorePredicate(String  value){
        return (band) -> band.getGenre().getName().compareTo(value)>0;
    }
    ...
    private Predicate<MusicBand> createEqualPredicate(String value){
        return (band) -> band.getGenre().getName().equalsIgnoreCase(value);
    }

    private Predicate<MusicBand> createInequalPredicate(String value){
        return (band) -> !band.getName().equalsIgnoreCase(value);
    }
}
